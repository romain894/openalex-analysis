...
from EntitiesConceptsAnalysis import get_name_of_entitie_from_api
...
def update_results_reference_analysis(set_progress,
    ...
    return fig_nb_time_referenced, table_most_cited_concept, raw_array, table_sum_all_entities, table_h_used_all_l_use_main, enriched_array
...
def update_table(active_cell, data):
    div_children = "Click the table"
    if active_cell:
        div_children = html.A(data[active_cell['row']]['Element names'], href=data[active_cell['row']]['Element link'], target="_blank")
    return div_children